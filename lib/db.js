...
function ensureSchema(db) {
  ...
  ensureColumn(db, "clients", "vehicle_plate", "TEXT");
}
...
export function createClient({ name, phone, email, vehiclePlate }) {
  const db = getConnection();
  db.prepare(
    "INSERT INTO clients (name, phone, email, vehicle_plate) VALUES (?, ?, ?, ?)"
  ).run(name, phone || null, email || null, vehiclePlate || null);
  return listClients();
}

export function updateClient({ id, name, phone, email, vehiclePlate }) {
  const db = getConnection();
  const result = db
    .prepare(
      `UPDATE clients
    ...
    .run(name, phone || null, email || null, vehiclePlate || null, id);
